  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      if (variantId) {
        ...
        const movement = await tx.stockMovement.create({
          data: {
            ...
            previousStock,
            newStock,
            notes: notes || "Ingreso manual de mercadería",
            createdById: userId,
      ...
      if (product.variants.length > 0) {
        throw new Error(
          "Este producto tiene variantes. Debés ingresar stock sobre una variante."
        );
      }
    ...
    console.error("Error en ingreso manual de stock:", error);
    return Response.json(
