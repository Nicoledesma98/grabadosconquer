...
export default function AdminStockVincularPage() {
  ...
  const [supplierCode, setSupplierCode] = useState("SUR");
  ...
  useEffect(() => {
    ...
    if (urlVariantId) setSelectedVariantId(urlVariantId);
  }, [searchParams]);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!externalSku.trim()) {
      setError("Ingresá el SKU externo del proveedor");
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/admin/stock/link-supplier", {
        method: "POST",
        headers: {
      ...
      setMessage("Vinculación guardada correctamente ✅");
      setExternalSku("");
      setExternalId("");
    } catch (e: any) {
      setError(e.message || "Error guardando vínculo");
    } finally {
      setSaving(false);
    }
  }
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Asociá un producto o variante interna con un SKU externo del
            proveedor
          </p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
        <section className="rounded-3xl border border-conquer-pink bg-white p-4">
          <h2 className="text-lg font-semibold text-conquer-navy mb-4">
            1. Elegí producto o variante interna
          </h2>
        ...
        <section className="rounded-3xl border border-conquer-pink bg-white p-4">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                value={externalSku}
                onChange={(e) => setExternalSku(e.target.value.toUpperCase())}
                className="h-11 w-full rounded-2xl border border-conquer-pink px-4"
                placeholder="BP161-02"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl border border-conquer-pink px-4"
                placeholder="ID opcional del proveedor"
              />
            </div>
